...
func (c *ReplicaCalculator) GetExternalMetricReplicas(currentReplicas int32, lowMark int64, highMark int64,  metricName string, wpa *v1alpha1.WatermarkPodAutoscaler, selector *metav1.LabelSelector) (replicaCount int32, utilization int64, timestamp time.Time, err error) {
	...
	log.Info(fmt.Sprintf("About to compare utilization %f vs LWM %d and HWM %d", adjustedUsage, lowMark, highMark))
	...
	if adjustedUsage > adjustedHM {
		...
		replicaCount = int32(math.Ceil(float64(currentReplicas) * adjustedUsage / (float64(lowMark))))
		...
		log.Info(fmt.Sprintf("Withing bounds of the watermarks. Value: %f is [%d; %d]", milliAdjustedUsage , lowMark, highMark))
