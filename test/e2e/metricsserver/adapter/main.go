...
// Package main is the fake custom/external metrics adapter used by the WPA e2e test suite.
// It is vendored (with a ConfigMap-based external-metrics injection patch, see provider/provider.go)
// from sigs.k8s.io/custom-metrics-apiserver's test-adapter sample, rather than depending on a
// separately maintained fork image, so the WPA repo owns and builds its own e2e test fixture image.
package main
...
import (
	...
	fakeprov "github.com/DataDog/watermarkpodautoscaler/test/e2e/metricsserver/adapter/provider"
)
